...
async function handleDecision(event, deps) {
  ...
  try {
    const approval = await applyApprovalDecision(approvalId, decisionAction, { decidedBy: auth.session.userId, decisionNote, organizationId, subjectType }, { ...deps, actorId: auth.session.userId });
