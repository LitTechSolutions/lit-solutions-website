...
- **MFA enrollment is the exception to best-effort email.** First enrollment requires successful delivery and explicit confirmation of a server-recorded, expiring, enrollment-bound challenge. If email is unavailable, `mfa-enroll.js` returns 503 without enabling MFA or issuing a session. Disable/reset notifications remain best-effort and audit their delivery outcome. Deploy migration `006_atomic_mfa_enrollment_challenges.sql` before deploying these functions.

**New, 2026-07-16 — Cloudinary object storage for `documents.js` (customer paperwork) and `admin-images.js` (image library), via `netlify/functions/_lib/cloudinary_store.js`:**

- `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET` — from the Cloudinary console (Dashboard → Account Details). The cloud name and API key are not especially sensitive on their own (Cloudinary's own client-side upload widgets routinely expose them), but the API secret must never leave the server — never in frontend code, never committed, never logged. Set all three in Netlify's Site settings → Environment variables (production) and in local `.env` (gitignored) for development.
- `documents.js` uploads with Cloudinary's `authenticated` delivery type (not reachable by a bare URL) and mints a fresh, short-lived signed URL (`signedUrlForPrivateAsset`) on every authorized read — customer paperwork is private data. `admin-images.js` uses ordinary public `upload` delivery, since those images are meant to end up on public pages anyway.
- **Fail behavior:** if any of the three variables is unset, `cloudinary_store.js` throws immediately on first use (fails closed, surfaced as a 502 from both endpoints' upload actions) rather than silently falling back to storing raw base64 again.
- **Rotation:** regenerating the API secret in Cloudinary's console immediately invalidates the old one — update the Netlify env var in the same sitting to avoid an outage window on document/image uploads and signed-URL generation. Uploaded assets themselves are unaffected by a secret rotation (only the credential used to talk to the API changes).
- Records written before this migration (raw `fileDataUri`/`dataUri` still in the Blobs record) are served as-is rather than broken by it — see `resolveFileUrl`/`resolveUrl` in the respective handlers.
