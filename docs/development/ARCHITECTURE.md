...
> **Status note (added 2026-07-19):** This document is Session 0's (2026-07-14) discovery-and-proposal snapshot, describing the codebase *before* any Care Hub code existed — treat §1/§2 as a historical baseline, not current state. Most of §3's target proposal was subsequently decided and built: Postgres/Neon (§3.3) is live with 6 migrations applied; a real React/Vite/TypeScript SPA (`care-hub-app/`) was built, superseding §3.1's hash-router-per-HTML-file recommendation; the actual frontend test tooling landed on Vitest + React Testing Library, not §3.8's `node --test`/Playwright suggestion (the existing root `node --test` suite continues to cover backend/domain logic). `admin.html` — referenced throughout §1.1/§1.2/§2 as one of "the only authenticated surfaces" and the subject of a boilerplate-deduplication recommendation in §3.1 — was deleted entirely on 2026-07-19; every capability it had is now a Care Hub screen, reusing the same backend functions. See `DEV_INDEX.md`'s status note for the fuller list of what else shipped after this corpus stopped being updated (2026-07-16), including an important unresolved production question (`DATABASE_URL`) that this document has no bearing on either way.
